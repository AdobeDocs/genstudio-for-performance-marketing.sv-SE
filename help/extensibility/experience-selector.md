...
När en [!DNL Experience] har valts i MFE och användaren klickar på knappen **[!UICONTROL Use]** måste en callback-funktion anropas för att skicka information om den markerade [!DNL Experience].

Återanropet innehåller en HTML-sträng som representerar [!DNL Experience]. Om Experience innehåller resurser (t.ex. bilder, videor) kommer HTML-element, som `<img>` och `<video>`, att innehålla `src`-attribut som innehåller försignerade URL:er för dessa resurser. Dessa försignerade URL:er har en förfallotid på 30 minuter och [!DNL Experience] bör användas av klientprogrammet under den tiden.

### Utdataschema

Callback-funktionen `onSelectionConfirmed` tar emot en array med [!DNL Experience] objekt med följande struktur:

| Egenskap | Typ | Beskrivning |
|----------|------|-------------|
| `id` | `string` | Unik identifierare för [!DNL Experience] (URN-format). |
| `content` | `string` | Base64-kodad HTML-sträng som representerar [!DNL Experience]. |
| `experienceFields` | `object` | Nyckelvärdepar för redigerbara fält i [!DNL Experience]. Varje fält i `experienceFields` innehåller: <ul><li>`fieldName`: Fältets namn/identifierare.</li><li>`fieldValue`: Fältets faktiska innehåll/värde.</li></ul> |
| `metadata` | `object` | Metadata om upplevelsen (t.ex. `channel`, `externalTemplateMetadata`, `externalAssetMetadata` osv.) |
| `aspectVariants` | `array` | Matris med proportionsvarianter för [!DNL Experience]. Varje variant innehåller: <ul><li>`aspectKey`: Unik identifierare för varianten.</li><li>`aspectMetadata`: Metadata, inklusive `channel` och `aspectRatio`.</li><li>`content`: Base64-kodad HTML-sträng för den här specifika proportionen.</li></ul> |

### Exempel-JSON

Nedan visas ett exempel på ett JSON-block som visar scheman.
